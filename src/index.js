...
module.exports = function(app, watchFile, conf = {}) {
    watchFiles = Array.isArray(watchFile) ? watchFile : [watchFile];
    ...
    app.all('/*', function(req, res, next) {
        ...
        const proxyFuzzyMatch = proxyNames.filter(function(kname) {
            const reg = new RegExp('^' + kname.replace(/(:\S*)[^/]/gi, '(\\S*)[^/]').replace(/\/\*$/, ''));
        ...
        const containMockURL = Object.keys(proxy).filter(function(kname) {
            ...
            return new RegExp('^' + kname.replace(/(:\S*)[^/]/gi, '(\\S*)[^/]') + replaceStr).test(proxyURL);
        });
    ...
    return {
        watch: (paths) => {
            const newWatchPaths = Array.isArray(paths) ? paths : [paths];
            watchFiles = watchFiles.concat(newWatchPaths);
            proxy = getProxy();
            watcher.add(paths);
        },
        unwatch: (paths) => {
            const newWatchPaths = Array.isArray(paths) ? paths : [paths];
            watchFiles = watchFiles.filter((filePath) => newWatchPaths.indexOf(filePath) === -1);
            proxy = getProxy();
